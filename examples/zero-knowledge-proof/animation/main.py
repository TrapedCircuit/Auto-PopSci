...
# ── Subtitle mixin ──────────────────────────────────────────────────────────
class SubtitleMixin:
    """Subtitle bar with TTS-driven wait + timestamp logging for audio merge."""
    ...
    def _log_timestamp(self):
        """Record the current renderer time for this show_sub call."""
        name = self.__class__.__name__
    ...
    def show_sub(self, text: str, *, wait: float = -1, font_size: int = 26):
        """Show subtitle, log timestamp, wait for TTS duration."""
        tts_dur = self._get_seg_duration()

        # Log the exact Manim timeline position for audio merge
        self._log_timestamp()

        # Show subtitle visually
        new_txt = Text(
